...
import Link from "next/link"; // Import Link untuk navigasi ke halaman register
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage("Login berhasil!"); // Pesan sukses
        router.push("/"); // Redirect ke halaman home setelah login berhasil
        router.refresh(); // Paksa refresh server components (Navbar) agar cookie terbaru terbaca
      } else {
        const error = await res.json();
        setMessage(error.message || "Login gagal"); // Pesan error
      }
    } catch (error) {
      ...
      <div className="max-w-md w-full space-y-8 bg-white bg-opacity-90 p-8 rounded-lg shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50" // Tombol biru seperti register
          >
            {loading ? "Masuk..." : "Masuk"}
          </button>
